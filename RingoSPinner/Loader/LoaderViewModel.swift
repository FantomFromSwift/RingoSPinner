...
final class LoaderViewModel: ObservableObject {
    ...
    let link = "https://coolsterclickrho.click/v1/public/install" // ИЗМЕНИТЬ
    ...
    private let bundleIdToSend = "6760655386" // ИЗМЕНИТЬ
    ...
    init() {
        ...
        if let locked = loadLock() {
            // Наличие lock = install был выполнен. Исправляем возможное несоответствие.
            if !UserDefaults.standard.bool(forKey: installCompletedKey) {
                UserDefaults.standard.set(true, forKey: installCompletedKey)
            }
            // Если device тоже не был сохранён — генерируем фолбэк
            let savedDevice = UserDefaults.standard.string(forKey: installDeviceKey) ?? ""
            if savedDevice.isEmpty {
                UserDefaults.standard.set(UUID().uuidString, forKey: installDeviceKey)
            }
...
//        ?? AppsFlyerLib.shared().getAppsFlyerUID()
    ...
    func allowOnceFromLink() async {
        ...
        // Ставим resolved сразу — предотвращаем повторный запуск пока ждём FCM
        resolved = true

        print("🧩 [LoaderVM] allowOnceFromLink start")
        isSplash = true
        errorMessage = nil
        ...
        // Запускаем все проверки параллельно, включая ожидание FCM
        // FCM стартует одновременно с ATT — получает максимум времени
        async let pushTask: () = waitForPushAnswer(timeoutSeconds: 0.4)
        async let attTask = waitForATTResolved(timeoutSeconds: 2.0)
        async let fcmTask = waitForFirstFCMToken(timeoutSeconds: 3.0)

        _ = await pushTask
        let (attAuthorized, deviceFromATT) = await attTask
        let fcmResult = await fcmTask

        // Если FCM не пришёл за 3с — продолжаем с пустым токеном.
        // Observer fcmTokenDidUpdate вызовет tryRefreshIfPossible когда токен придёт позже.

        print("🧩 [LoaderVM] Проверки завершены. ATT: \(attAuthorized), FCM: \(fcmResult ?? "nil")")
        ...
        do {
            ...
            if !finalFCM.isEmpty {
                ...
                // Токен не пришёл во время сплеша — опрашиваем каждые 3с до 30с
                Task {
                    let retryDeadline = Date().addingTimeInterval(30)
                    while Date() < retryDeadline {
                        try? await Task.sleep(nanoseconds: 3_000_000_000)
                        let delayed = UserDefaults.standard.string(forKey: self.fcmKey) ?? ""
                        if !delayed.isEmpty {
                            print("🧩 [LoaderVM] delayedFCMRetry: found token, sending refresh")
                            self.tryRefreshIfPossible(candidateToken: delayed, reason: "delayedFCMRetry")
                            break
                        }
                        print("🧩 [LoaderVM] delayedFCMRetry: no token yet, retrying...")
                    }
                }
            }      
